...
roiSettingsSchema.pre('save', function () {
  ...
  if (!this.averageHourlyCost) {
    this.averageHourlyCost = Math.round(
      (this.averageSalary * this.overheadMultiplier) / hoursPerYear
    );
  }
  ...
  if (!this.costFactors.decisionDelayCostPerDay) {
    this.costFactors.decisionDelayCostPerDay = Math.round(
      this.averageHourlyCost * this.hoursPerDay
    );
  }

  if (!this.costFactors.focusTimeLossCostPerPct) {
    // Cost of 1% focus time loss per person per week
    this.costFactors.focusTimeLossCostPerPct = Math.round(
      this.averageHourlyCost * this.hoursPerDay * 5 * 0.01
    );
  }
