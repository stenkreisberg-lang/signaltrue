...
export async function updateROISettings(orgId, updates, userId) {
  ...
  if (
    updates.averageSalary ||
    updates.workingDaysPerYear ||
    updates.hoursPerDay ||
    updates.overheadMultiplier
  ) {
...
async function calculateFocusTimeSavings(orgId, settings, periodDays = 30) {
  ...
  const avgFocusRatio =
    metrics.reduce((sum, m) => sum + (m.focusTimeRatio || 0.5), 0) / metrics.length;

  // Baseline is 40% focus time, savings if above
  const baselineFocusRatio = 0.4;
  const focusGainPct = Math.max(0, avgFocusRatio - baselineFocusRatio) * 100;

  // Each 1% focus gain = significant productivity boost
  const weeksInPeriod = periodDays / 7;
  const savings = Math.round(
    focusGainPct * settings.teamSize * weeksInPeriod * settings.costFactors.focusTimeLossCostPerPct
  );
...
async function calculateInterventionROI(orgId, settings, periodDays = 90) {
  ...
  const savings = Math.round(
    avgImprovement * settings.costFactors.meetingHourCost * settings.teamSize * 4
  );
...
export async function calculateROISummary(orgId, options = {}) {
  ...
  const totalSavings =
    meetingLoad.savings + focusTime.savings + responseTime.savings + interventions.savings;
  ...
  for (const drift of activeDrifts) {
    ...
    switch (drift.metricType) {
      case 'meetingLoadIndex':
        ...
        costPerDay =
          ((drift.magnitude || 2) * settings.teamSize * settings.costFactors.meetingHourCost) / 7;
        break;
      case 'focusTimeRatio':
        ...
        costPerDay =
          ((drift.magnitude || 5) *
            settings.teamSize *
            settings.costFactors.focusTimeLossCostPerPct) /
          7;
        break;
...
export async function getROIDashboardBanner(orgId) {
  ...
  const periodDays =
    settings.roiPeriod === 'weekly'
      ? 7
      : settings.roiPeriod === 'monthly'
        ? 30
        : settings.roiPeriod === 'quarterly'
          ? 90
          : 365;
