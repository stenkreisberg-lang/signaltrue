...
export async function calculateManagerEffectiveness(managerId, teamId) {
  try {
    ...
    let record = await ManagerEffectiveness.findOne({ managerId, teamId }).sort({ createdAt: -1 });
...
async function getOrgBenchmarks(orgId) {
  try {
    ...
    const avgEffectiveness =
      allManagers.reduce((sum, m) => sum + m.effectivenessScore, 0) / allManagers.length;
    const avgTeamHealth =
      allManagers.reduce((sum, m) => sum + (m.teamOutcomes?.teamHealthTrend || 0), 0) /
      allManagers.length;
    const avgRetention =
      allManagers.reduce((sum, m) => sum + (m.teamOutcomes?.teamRetention || 85), 0) /
      allManagers.length;
...
function generateCoachingPlan(improvementAreas) {
  const coachingRecommended = improvementAreas.some(
    (area) => area.impact === 'critical' || area.impact === 'high'
  );
