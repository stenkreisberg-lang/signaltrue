...
const SIGNAL_DEFINITIONS = {
  ...
  execution_stagnation: {
    ...
    explanation:
      'Work completion dropped while open work kept accumulating. Risk: chronic backlog pressure.',
    recommendedActions: [
      {
  ...
  rework_spiral: {
    ...
    explanation:
      'Reopened work increased. This usually indicates unclear requirements, rushed delivery, or unstable priorities.',
    recommendedActions: [
      {
  ...
  overcommitment_risk: {
    ...
    explanation:
      'Overdue work is increasing and spillover appears after hours. Risk of sustained overload.',
    recommendedActions: [
      {
  ...
  wip_overload: {
    ...
    explanation:
      'Work-in-progress is accumulating beyond sustainable levels. Context switching is likely impacting delivery.',
    recommendedActions: [
      {
  ...
  boundary_erosion: {
    ...
    researchBacking: {
      ...
      citation:
        'After-hours email expectations associate with emotional exhaustion and poorer work-family balance',
      link: null,
    },
    triggers: [
    ...
    explanation:
      'After-hours email increased vs your normal pattern. This is a leading indicator of overload.',
    recommendedActions: [
      {
  ...
  panic_coordination: {
    ...
    explanation:
      'Coordination is increasing while completion is decreasing. Likely root cause: unclear decisions, shifting priorities.',
    recommendedActions: [
      {
  ...
  meeting_fatigue: {
    ...
    explanation:
      'Back-to-back meetings are compressing recovery time. This pattern precedes focus erosion.',
    recommendedActions: [
      {
  ...
  response_drift: {
    ...
    triggers: [{ metric: 'replyLatencyDrift', operator: '>=', threshold: 30 }],
    explanation:
      'Response times are slowing compared to baseline. This may indicate capacity constraints or communication overload.',
    recommendedActions: [
      {
  ...
  decision_churn: {
    ...
    explanation:
      'Decisions are being rewritten repeatedly while coordination spikes. This typically increases cognitive load on senior staff.',
    recommendedActions: [
      {
  ...
  documentation_decay: {
    ...
    triggers: [{ metric: 'orphanPages30d', operator: '>=', threshold: 20 }],
    explanation:
      'Documentation is being created but not maintained. This creates knowledge silos and onboarding friction.',
    recommendedActions: [
      {
  ...
  cognitive_overload: {
    ...
    explanation:
      'High context switching across documentation. This may indicate fragmented focus or unclear ownership.',
    recommendedActions: [
      {
  ...
  external_pressure_injection: {
    ...
    researchBacking: {
      ...
      citation:
        'External pressure cascading into execution teams creates unsustainable demand spikes',
      link: null,
    },
    triggers: [
    ...
    explanation:
      'Customer pressure increased and cascaded into execution overload. Recommended action: add intake gates, freeze scope, clarify priorities.',
    recommendedActions: [
      {
  ...
  escalation_cascade: {
    ...
    triggers: [{ metric: 'escalationRate7d', operator: '>=', threshold: 0.3 }],
    explanation:
      'High deal churn indicates unstable pipeline. This often cascades into delivery pressure.',
    recommendedActions: [
      {
  ...
  handoff_spike: {
    ...
    triggers: [{ metric: 'handoffSpike48h', operator: '>=', threshold: 5 }],
  ...
  recovery_collapse: {
    ...
    triggers: [{ metric: 'rci', operator: '>=', threshold: 65 }],
    explanation:
      'Recovery time is severely compromised across multiple indicators. Risk of burnout is elevated.',
    recommendedActions: [
      {
  ...
  work_aging_pressure: {
    ...
    triggers: [{ metric: 'wap', operator: '>=', threshold: 65 }],
    explanation:
      'Work backlog is aging rapidly across multiple dimensions. Chronic pressure is building.',
    recommendedActions: [
      {
  ...
  systemic_overload: {
    ...
    explanation:
      'Multiple indicators show systemic overload. Work is not completing, recovery is compromised, and backlog is aging.',
    recommendedActions: [
      {
  ...
  passive_disengagement: {
    ...
    researchBacking: {
      ...
      citation:
        'Silence in async environments is data - rising unanswered posts indicate withdrawal',
      link: null,
    },
    triggers: [
    ...
    explanation:
      'Engagement in async channels is declining. This may indicate disengagement or communication channel issues.',
    recommendedActions: [
      {
...
export async function detectSignals(orgId, options = {}) {
  ...
  const latestMetrics = await IntegrationMetricsDaily.findOne(query).sort({ date: -1 }).lean();
    ...
    if (conditionResult.met) {
      ...
      metricDeltas.push({
        ...
        deltaPercent: conditionResult.previousValue
          ...
          : 0,
        direction:
          currentValue > (conditionResult.previousValue || 0)
            ? 'up'
            : currentValue < (conditionResult.previousValue || 0)
              ? 'down'
              : 'stable',
...
function evaluateCondition(trigger, currentValue, historicalMetrics) {
  const previousValue = historicalMetrics.length > 1 ? historicalMetrics[0][trigger.metric] : null;
...
function calculateConfidence(connections, latestMetrics, definition) {
  ...
  if (
    taskConnection &&
    taskConnection.coverage?.mappedUsers / taskConnection.coverage?.totalUsers >= 0.7
  ) {
    confidence += 25;
  ...
  if (
    gmailConnection &&
    gmailConnection.coverage?.mappedUsers / gmailConnection.coverage?.totalUsers >= 0.7
  ) {
    confidence += 25;
...
export async function getActiveSignals(orgId, options = {}) {
  ...
  return CategoryKingSignal.find(query).sort({ severity: -1, confidence: -1 }).limit(limit).lean();
}
