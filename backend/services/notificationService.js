...
export async function sendSlackSummary(channelId, teamName, summary) {
  if (!slackClient) {
    console.warn('⚠️  Slack client not configured, skipping Slack notification');
    return false;
  }
  ...
  try {
      ...
      blocks: [
        ...
        },
        {
          ...
          elements: [
            {
              type: 'mrkdwn',
              text: `Generated by SignalTrue • ${new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}`,
            },
          ],
        },
      ],
    ...
    console.log(`✓ Slack summary sent for team: ${teamName}`);
    return true;
  } catch (error) {
    console.error(`❌ Slack send failed for ${teamName}:`, error.message);
    return false;
  }
}
...
<body>
  ...
  try {
    ...
    console.error(`❌ Email send failed for ${teamName}:`, error.message);
    return false;
  }
}
...
export async function sendWeeklySummaries(options = {}) {
  ...
  console.log('✅ All weekly summaries sent');
}
