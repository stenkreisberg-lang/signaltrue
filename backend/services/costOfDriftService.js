    ...
    return {
      ...
      breakdown: {
        meetingOverhead: {
          ...
          percentage:
            totalHoursLost > 0 ? Math.round((meetingHoursLost / totalHoursLost) * 100) : 0,
        },
        executionDelay: {
          hours: Math.round(executionDelayHours * 10) / 10,
          cost: Math.round(executionDelayHours * avgHourlyCost),
          percentage:
            totalHoursLost > 0 ? Math.round((executionDelayHours / totalHoursLost) * 100) : 0,
        },
        rework: {
...
export async function calculateOrgCostOfDrift(orgId, options = {}) {
  try {
    ...
    const sortedByImpact = [...validCosts].sort(
      (a, b) => b.weeklyEstimate.midpoint - a.weeklyEstimate.midpoint
    );
