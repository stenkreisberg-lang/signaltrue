...
export async function recordActionOutcome(experimentId) {
  try {
    ...
    console.log(
      `✅ Learning recorded: ${learning.outcome} outcome for ${learning.action.title} (${learning.teamProfile.function} team)`
    );
...
export async function getLearningStats(teamId) {
  try {
    ...
    const successRate = stats.reduce(
      (acc, s) => {
...
function extractDriversFromAction(action) {
  ...
  if (title.includes('after-hours') || title.includes('quiet hours'))
    drivers.push('after_hours_activity');
