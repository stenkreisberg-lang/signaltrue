...
function detectBehavioralSignals(email, teams, calls) {
  return {
    ...
    afterHoursWork: {
      ...
      percentOfTotal:
        ((email.afterHoursEmailCount + calls.afterHoursCallCount) /
          (email.sentCount + calls.callCount)) *
        100,
    },
    responsivenessDrop: {
...
function generateRecommendations(signals) {
  ...
  if (signals.emailOverload.detected) {
    recs.push(
      `${signals.emailOverload.unreadCount} unread emails - encourage inbox zero practices or email batching`
    );
  }

  if (signals.afterHoursWork.detected) {
    recs.push(
      `${signals.afterHoursWork.percentOfTotal.toFixed(0)}% of work happening after hours - risk of burnout`
    );
  }
