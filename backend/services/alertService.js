...
export async function sendDriftAlerts() {
  const events = await DriftEvent.find({
    acknowledged: false,
  ...
  for (const event of events) {
    ...
    const contributors = event.topContributors?.length
      ? event.topContributors
          .map((c) => `${c.metric} ${c.change > 0 ? '↑' : '↓'} ${Math.abs(Math.round(c.change))}%`)
          .join(', ')
      : '';
    ...
    if (emailTransporter && org.email) {
      try {
        await emailTransporter.sendMail({
          ...
          to: org.email,
          subject: 'SignalTrue Workload Risk Alert',
          text: msg,
          html: `<p>${msg.replace(/\n/g, '<br>')}</p>`,
        });
      } catch (e) {
