...
${signals
  .map(
    (s) => `- ${s.signal_type}: severity ${s.severity}, confidence ${s.confidence}
  Drivers: ${(s.drivers || []).join(', ')}
  Key changes: ${(s.metric_deltas || []).map((d) => `${d.metric}: ${d.delta_pct > 0 ? '+' : ''}${d.delta_pct.toFixed(0)}%`).join(', ')}`
  )
  .join('\n')}
...
function generateFallbackNarrative(signals, evidence) {
  ...
  const description =
    signalDescriptions[topSignal.signal_type] || 'a pattern worth monitoring has been detected';
  ...
  const confidenceNote =
    topSignal.confidence < 60 ? 'This is an early pattern, not a conclusion. ' : '';
  ...
  const summary = `${confidenceNote}The data shows ${description}. Specifically, ${topEvidence || 'key metrics have shifted'} compared to baseline. ${
    signals.length > 1
      ? `${signals.length - 1} additional pattern${signals.length > 2 ? 's' : ''} also detected.`
      : ''
  }`;
...
function buildConfidenceNotes(connectors, signals) {
  ...
  for (const [name, connector] of Object.entries(connectors || {})) {
    if (connector.connected && connector.coverage_pct < 70) {
      notes.push(
        `${formatConnectorName(name)} coverage is ${connector.coverage_pct}%, user mapping may be incomplete.`
      );
    }
  }
  ...
  if (lowConfidenceSignals.length > 0) {
    notes.push(
      'Some signals have low confidence due to limited data. These are early patterns, not conclusions.'
    );
  }
...
function getSignalPlainDescription(signalType) {
  const descriptions = {
    recovery_collapse:
      'recovery time patterns suggest the team may benefit from more breathing room',
...
export async function buildCopilotPayload(orgId, options = {}) {
  ...
  ['jira', 'asana', 'gmail', 'meet', 'notion', 'hubspot', 'pipedrive', 'basecamp'].forEach(
    (type) => {
      if (!connectors[type]) {
