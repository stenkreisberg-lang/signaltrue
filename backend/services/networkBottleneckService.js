...
const THRESHOLDS = {
  ...
  CRITICAL: 0.6, // 60% - severe concentration
};
...
export async function calculateNetworkBottleneck(teamId, periodDays = 7) {
  try {
    ...
    const concentrationProxy =
      recentMetrics.length > 0 ? calculateConcentration(recentMetrics) : 0.3;
    ...
    // Calculate delta
    const deltaPercent =
      baselineBottleneck > 0 ? (bottleneckScore - baselineBottleneck) / baselineBottleneck : 0;
    ...
    // Direction indicator
    const direction =
      deltaPercent > 0.05 ? 'worsening' : deltaPercent < -0.05 ? 'improving' : 'stable';
    ...
    return {
      ...
      networkHealth: {
        ...
        concentrationLevel:
          bottleneckScore >= 0.5 ? 'High' : bottleneckScore >= 0.35 ? 'Moderate' : 'Healthy',
      },
      ...
      suggestedAction:
        bottleneckScore >= THRESHOLDS.WARNING
...
function calculateConcentration(metrics) {
  ...
  const activities = metrics.map((m) => (m.messageCount || 0) + (m.meetingCount || 0) * 5);
  ...
  const variance =
    activities.reduce((sum, val) => sum + Math.pow(val - avg, 2), 0) / activities.length;
