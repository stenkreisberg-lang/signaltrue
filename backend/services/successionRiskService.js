...
function generateRecommendations(knowledge, successors, metrics) {
  ...
  if (criticalKnowledge.length > 0) {
    recs.push(
      `Document ${criticalKnowledge.length} critical knowledge area(s): ${criticalKnowledge.map((k) => k.topic).join(', ')}`
    );
  }
  ...
  if (metrics.questionersAsPercentOfTeam > 50) {
    recs.push(
      'Over 50% of team depends on this person - create documentation and training materials'
    );
  }
