...
export async function calculateContextSwitchingIndex(teamId, periodDays = 7) {
  try {
    ...
    const avgMeetingsPerDay = average(
      recentMetrics.map((m) => m.meetingsPerDay || m.meetingCount || 0)
    );
    const avgSlackThreadsPerDay = average(
      recentMetrics.map((m) => m.slackThreadsActive || m.messageCount / 10 || 0)
    );
    ...
    // Calculate delta
    const deltaPercent = baselineCSI > 0 ? (currentCSI - baselineCSI) / baselineCSI : 0;
    ...
    return {
      ...
      suggestedAction:
        deltaPercent >= THRESHOLDS.WARNING
