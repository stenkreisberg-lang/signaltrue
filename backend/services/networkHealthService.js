...
function generateRecommendations(silos, bottlenecks, knowledgeRisks) {
  ...
  if (bottlenecks.length > 0) {
    recs.push(
      `${bottlenecks.length} bottleneck(s) detected - distribute knowledge and decision-making`
    );
  }
  ...
  if (criticalKnowledge.length > 0) {
    recs.push(
      `${criticalKnowledge.length} critical knowledge risk(s) - implement knowledge sharing sessions`
    );
  }
...
export async function getOrgNetworkHealth(orgId) {
  try {
    ...
    for (const team of teams) {
      const health = await NetworkHealth.findOne({ teamId: team._id }).populate('teamId', 'name');
