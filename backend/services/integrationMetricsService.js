...
function computeTaskMetrics(events, date) {
  const taskEvents = events.filter((e) => ['jira', 'asana'].includes(e.source));
  ...
  const completedEvents = events7d.filter(
    (e) =>
      e.eventType === 'task_completed' ||
      (e.eventType === 'task_status_changed' &&
        ['done', 'closed', 'complete', 'resolved'].includes(
          e.metadata?.statusCurrent?.toLowerCase()
        ))
  );
  ...
  // Reopen rate
  const reopenRate7d = tasksCompleted7d > 0 ? tasksReopened7d / tasksCompleted7d : 0;
  ...
  const eventsPrev7d = taskEvents.filter(
    (e) => new Date(e.timestamp) >= window14dStart && new Date(e.timestamp) < window7dStart
  );
  const completedPrev7d = eventsPrev7d.filter(
    (e) =>
      e.eventType === 'task_completed' ||
      (e.eventType === 'task_status_changed' &&
        ['done', 'closed', 'complete', 'resolved'].includes(
  ...
  // Completion change %
  const completionChange7d =
    completedPrev7d > 0 ? ((tasksCompleted7d - completedPrev7d) / completedPrev7d) * 100 : 0;
  ...
  const overdueEvents = events7d.filter(
    (e) => e.metadata?.dueOn && new Date(e.metadata.dueOn) < date
  );
...
function computeEmailMetrics(events, date) {
  ...
  const externalEmailRatio = events7d.length > 0 ? externalEmails.length / events7d.length : 0;
...
function computeNotionMetrics(events, date) {
  ...
  const pageEdits = {};
  events7d
    .filter((e) => e.eventType === 'page_updated')
    .forEach((e) => {
  ...
  const highCollabPages7d = events7d.filter((e) => e.metadata?.collaboratorCount > 3).length;
...
function computeCRMMetrics(events, date) {
  const crmEvents = events.filter((e) => ['hubspot', 'pipedrive'].includes(e.source));
  ...
  events7d
    .filter((e) => e.eventType === 'deal_stage_changed')
    .forEach((e) => {
      if (e.metadata?.dealIdHash) {
        dealStageChanges[e.metadata.dealIdHash] =
          (dealStageChanges[e.metadata.dealIdHash] || 0) + 1;
      }
    });
  const dealsWithMultipleStageChanges7d = Object.values(dealStageChanges).filter(
    (c) => c > 2
  ).length;
  const totalDeals = Object.keys(dealStageChanges).length;
  const escalationRate7d = totalDeals > 0 ? dealsWithMultipleStageChanges7d / totalDeals : 0;
...
function computeBasecampMetrics(events, date) {
  ...
  const unansweredPostRate7d =
    postsCreated7d > 0
      ? Math.max(
          0,
          1 - events7d.filter((e) => e.eventType === 'comment_added').length / postsCreated7d
        )
      : 0;
...
function computeCompositeMetrics(taskMetrics, emailMetrics, meetingMetrics, crmMetrics, baseline) {
  ...
  const interruptEvents7d =
    (meetingMetrics.adHocMeetingCount7d || 0) +
  ...
  const gapInverse =
    meetingMetrics.avgGapBetweenMeetingsMins > 0
  ...
  const overdueGrowthZ = computeRobustZ(taskMetrics.overdueGrowth7d || 0, 0, 3);
  ...
  const handoffZ = computeRobustZ(crmMetrics.handoffSpike48h || 0, 0, 2);
...
async function getTrailingBaseline(orgId, teamId, userId, date, days) {
  ...
  const historicalMetrics = await IntegrationMetricsDaily.find(query).sort({ date: -1 }).lean();
...
function buildDataCoverage(connections) {
  ...
  coverage.userMappingPercent = totalUsers > 0 ? Math.round((totalMapped / totalUsers) * 100) : 0;
