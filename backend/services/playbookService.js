...
export function getRecommendation(metric, direction) {
  const playbooks = {
    sentiment: {
      negative:
        'Tone drop detected → Encourage recognition posts or 1:1 check-ins to surface concerns.',
      positive: 'Tone improving → Celebrate wins and maintain momentum with team shout-outs.',
    },
    response: {
      negative:
        'Response latency rising → Check workload distribution and consider reducing meeting load.',
    ...
    network: {
      ...
      positive:
        'Network breadth expanding → Team engaging broadly; foster continued collaboration.',
    },
    focus: {
    ...
    energy: {
      negative:
        'Energy Index dropping → Review top contributors and implement targeted interventions.',
