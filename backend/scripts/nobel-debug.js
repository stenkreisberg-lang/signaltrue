...
for (const key of Object.keys(ints)) {
  ...
  if (val && typeof val === 'object' && Object.keys(val).length > 0) {
    console.log(key + ':', JSON.stringify({ hasToken: !!val.accessToken, scope: val.scope, installed: val.installed, email: val.email }, null, 2));
  }
}
...
const sources = await WorkEvent.aggregate([
  { $match: { orgId: org._id } },
  { $group: { _id: { source: '$source', eventType: '$eventType' }, count: { $sum: 1 }, earliest: { $min: '$timestamp' }, latest: { $max: '$timestamp' } } },
  { $sort: { count: -1 } }
]);

for (const s of sources) {
  console.log(`  ${s._id.source} / ${s._id.eventType}: ${s.count} events (${new Date(s.earliest).toLocaleDateString()} → ${new Date(s.latest).toLocaleDateString()})`);
}
...
for (const w of weeklyBreakdown) {
  console.log(`  Week ${w._id.week}/${w._id.year}: ${w._id.source}/${w._id.eventType} = ${w.count}`);
}
