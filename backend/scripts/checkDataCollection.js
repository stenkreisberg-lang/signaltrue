...
async function check() {
  ...
  const user = await mongoose.connection.db
    .collection('users')
    .findOne({ email: 'sten.kreisberg@signaltrue.ai' });
  ...
  console.log(
    'Google expiry:',
    user.google?.expiry_date ? new Date(user.google.expiry_date) : 'N/A'
  );
  ...
  // Check signals_v2
  const signalsV2 = await mongoose.connection.db
  ...
  // Check baselines
  const baselines = await mongoose.connection.db
  ...
  const loopClosing = await mongoose.connection.db
    .collection('loopclosings')
    .find({ orgId: user.orgId })
    .toArray();
