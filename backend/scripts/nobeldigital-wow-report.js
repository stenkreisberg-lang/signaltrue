...
try {
  IntegrationMetricsDaily = (await import('../models/integrationMetricsDaily.js')).default;
} catch {}
try {
  MetricsDaily = (await import('../models/metricsDaily.js')).default;
} catch {}
...
async function main() {
  ...
  const org = await Organization.findOne({
    $or: [{ slug: /nobel/i }, { name: /nobel/i }, { domain: /nobel/i }],
  });
  ...
  if (allKeys.length === 0) {
    ...
    console.log(
      `  ${'Source / Type'.padEnd(35)} ${'Last Week'.padStart(10)} ${'This Week'.padStart(10)} ${'Change'.padStart(10)}`
    );
    ...
    for (const key of allKeys) {
      ...
      console.log(
        `  ${key.padEnd(35)} ${String(lw).padStart(10)} ${String(tw).padStart(10)} ${pct(tw, lw).padStart(10)}`
      );
    }
    console.log('  ' + '─'.repeat(65));
    console.log(
      `  ${'TOTAL'.padEnd(35)} ${String(lwTotal).padStart(10)} ${String(twTotal).padStart(10)} ${pct(twTotal, lwTotal).padStart(10)}`
    );
  }
  ...
  if (IntegrationMetricsDaily) {
    ...
    if (metricsThisWeek.length === 0 && metricsLastWeek.length === 0) {
      ...
      console.log(
        `  Data points: This week ${metricsThisWeek.length}, Last week ${metricsLastWeek.length}`
      );
      console.log(
        `  ${'Metric'.padEnd(28)} ${'Last Week'.padStart(12)} ${'This Week'.padStart(12)} ${'Change'.padStart(10)} ${''.padStart(3)}`
      );
      console.log('  ' + '─'.repeat(65));
      ...
      for (const [field, label, higherIsBad] of fields) {
        ...
        console.log(
          `  ${label.padEnd(28)} ${lw.toFixed(2).padStart(12)} ${tw.toFixed(2).padStart(12)} ${pct(tw, lw).padStart(10)} ${f}`
        );
      }
  ...
  console.log(
    `  Signals detected: Last week ${signalsLastWeek.length} → This week ${signalsThisWeek.length} ${pct(signalsThisWeek.length, signalsLastWeek.length)}`
  );
  ...
  for (const sev of ['Critical', 'Risk', 'Informational']) {
    ...
    if (tw > 0 || lw > 0) {
      console.log(
        `    ${sev.padEnd(18)} ${String(lw).padStart(5)} → ${String(tw).padStart(5)} ${pct(tw, lw).padStart(10)} ${flag(tw, lw, true)}`
      );
    }
  }
  ...
  if (allTypes.length > 0) {
    ...
    for (const t of allTypes) {
      ...
      console.log(
        `    ${t.padEnd(30)} ${String(lw).padStart(5)} → ${String(tw).padStart(5)} ${pct(tw, lw).padStart(10)}`
      );
    }
  }
  ...
  console.log(
    `  High severity (≥70): ${highLW.length} → ${highTW.length} ${flag(highTW.length, highLW.length, true)}`
  );
  ...
  if (highTW.length > 0) {
    ...
    for (const s of highTW.sort((a, b) => b.severity - a.severity)) {
      console.log(
        `    [${s.severity}] ${s.signalType} — ${s.explanation?.substring(0, 80) || 'No explanation'}`
      );
    }
  }
  ...
  for (const team of teams) {
    ...
    if (latestBDI) {
      ...
      const drivers = (latestBDI.drivers || [])
        .slice(0, 3)
        .map((d) => `${d.signal}(${d.contribution}%)`)
        .join(', ');
      console.log(`  📋 ${team.name}`);
      console.log(
        `     BDI Score: ${prev} → ${curr}/100  |  State: ${state}  |  Confidence: ${conf}`
      );
  ...
  if (MetricsDaily) {
    ...
    for (const team of teams) {
      ...
      for (const [field, label, bad] of legacyFields) {
        ...
        if (tw != null || lw != null) {
          console.log(
            `     ${label.padEnd(22)} ${(lw ?? 0).toFixed(2).padStart(10)} → ${(tw ?? 0).toFixed(2).padStart(10)} ${pct(tw ?? 0, lw ?? 0).padStart(10)} ${flag(tw ?? 0, lw ?? 0, bad)}`
          );
        }
      }
    }
  ...
  const checkInt = (name, path) => {
    ...
    console.log(
      `  ${hasToken ? '✅' : '❌'} ${name.padEnd(18)} ${hasToken ? 'Connected' : 'Not connected'}${lastSync ? ` | Last sync: ${new Date(lastSync).toLocaleString()}` : ''}${status ? ` | Status: ${status}` : ''}${events != null ? ` | Events: ${events}` : ''}`
    );
  };
