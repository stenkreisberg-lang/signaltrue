...
const withUser = await WorkEvent.countDocuments({ orgId: org._id, source: 'microsoft-outlook', actorUserId: { $ne: null } });
console.log(`\nAfter re-sync:`);
console.log(`  Total outlook events: ${total}`);
console.log(`  With userId set:      ${withUser}  (${total > 0 ? Math.round(withUser/total*100) : 0}%)`);
console.log(`  Without userId:       ${total - withUser}`);

// Quick per-user hour breakdown
const perUser = await WorkEvent.aggregate([
  { $match: { orgId: org._id, source: 'microsoft-outlook', eventType: 'meeting', 'metadata.durationMinutes': { $gt: 0 } } },
  { $group: { _id: '$actorUserId', totalMinutes: { $sum: '$metadata.durationMinutes' }, meetings: { $sum: 1 } } },
  { $sort: { totalMinutes: -1 } }
]);
...
for (const u of perUser.slice(0, 10)) {
  ...
  console.log(`  ${(info?.name || 'unknown').padEnd(25)} ${(u.totalMinutes/60).toFixed(1).padStart(5)}h  (${u.meetings} meetings)`);
}
if (perUser.length > 10) console.log(`  ... and ${perUser.length - 10} more`);
console.log(`\n  No userId: ${perUser.filter(u => !u._id).map(u => (u.totalMinutes/60).toFixed(1)+'h').join(', ') || 'none'}`);
