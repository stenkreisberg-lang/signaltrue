...
console.log(`    This week  (${thisWeekStart.toLocaleDateString()} – today):      ${twUserEvents.length} users`);
console.log(`    Last week  (${lastWeekStart.toLocaleDateString()} – ${thisWeekStart.toLocaleDateString()}): ${lwUserEvents.length} users`);
...
const lwMeetingsBySource = await WorkEvent.aggregate([
  { $match: { orgId: org._id, eventType: 'meeting', timestamp: { $gte: lastWeekStart, $lt: thisWeekStart } } },
  { $group: { _id: '$source', count: { $sum: 1 }, uniqueUsers: { $addToSet: '$userId' } } }
]);
...
for (const s of twMeetingsBySource) {
  console.log(`    ${s._id}: ${s.count} events, ${s.uniqueUsers.filter(Boolean).length} distinct users`);
}
...
const twMeetingHours = await WorkEvent.aggregate([
  { $match: { orgId: org._id, eventType: 'meeting', timestamp: { $gte: thisWeekStart, $lte: now }, 'metadata.durationMinutes': { $gt: 0 } } },
  { $group: {
    _id: '$userId',
...
const lwMeetingHours = await WorkEvent.aggregate([
  { $match: { orgId: org._id, eventType: 'meeting', timestamp: { $gte: lastWeekStart, $lt: thisWeekStart }, 'metadata.durationMinutes': { $gt: 0 } } },
  { $group: {
    _id: '$userId',
    totalMinutes: { $sum: '$metadata.durationMinutes' },
    meetingCount: { $sum: 1 }
...
console.log(`    Avg per contributing user:${twUsersWithHours > 0 ? (twTotalHours / twUsersWithHours).toFixed(1) : '—'}h/week`);
console.log(`    Avg across ALL ${totalUsers} users:  ${(twTotalHours / totalUsers).toFixed(1)}h/person/week`);
console.log(`    LAST WEEK:`);
console.log(`    Total org hours:          ${lwTotalHours.toFixed(1)}h`);
console.log(`    Users contributing hours: ${lwUsersWithHours}`);
console.log(`    Avg per contributing user:${lwUsersWithHours > 0 ? (lwTotalHours / lwUsersWithHours).toFixed(1) : '—'}h/week`);
console.log(`    Avg across ALL ${totalUsers} users:  ${(lwTotalHours / totalUsers).toFixed(1)}h/person/week`);
...
for (const u of sorted.slice(0, 20)) {
  ...
  console.log(`    ${name.slice(0,29).padEnd(30)} ${email.slice(0,39).padEnd(40)} ${(u.totalMinutes/60).toFixed(1).padStart(5)}h  ${String(u.meetingCount).padStart(3)}`);
}
if (sorted.length > 20) console.log(`    ... and ${sorted.length - 20} more users`);

// ── 6. Events with no userId ──
const noUserEvents = await WorkEvent.countDocuments({ orgId: org._id, eventType: 'meeting', userId: null, timestamp: { $gte: thisWeekStart } });
console.log(`\n[6] MEETING EVENTS WITH NO userId THIS WEEK: ${noUserEvents}`);

// ── 7. IntegrationMetricsDaily records ──
const twMetrics = await IntegrationMetricsDaily.find({ orgId: org._id, date: { $gte: thisWeekStart, $lte: now } }).lean();
const lwMetrics = await IntegrationMetricsDaily.find({ orgId: org._id, date: { $gte: lastWeekStart, $lt: thisWeekStart } }).lean();
console.log(`\n[7] IntegrationMetricsDaily RECORDS`);
...
for (const m of twMetrics.slice(0, 5)) {
  console.log(`    date:${m.date?.toISOString().split('T')[0]}  meetingHours7d:${m.meetingDurationTotalHours7d?.toFixed(1)}  eventsProcessed:${m.eventsProcessed}  teamId:${m.teamId || 'null(org)'}`);
}
...
console.log(`  Coverage TW (of ${totalUsers} users):       ${totalUsers > 0 ? Math.round(twUsersWithHours/totalUsers*100) : 0}%`);
console.log(`  Coverage LW (of ${totalUsers} users):       ${totalUsers > 0 ? Math.round(lwUsersWithHours/totalUsers*100) : 0}%`);
console.log(`  Total org meeting hours TW:      ${twTotalHours.toFixed(1)}h`);
console.log(`  Per-person avg TW (connected):   ${twUsersWithHours > 0 ? (twTotalHours/twUsersWithHours).toFixed(1) : '—'}h`);
console.log(`  Per-person avg LW (connected):   ${lwUsersWithHours > 0 ? (lwTotalHours/lwUsersWithHours).toFixed(1) : '—'}h`);
