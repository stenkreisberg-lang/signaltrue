...
async function checkIntegrationStatus() {
  ...
  const query = orgFilter ? { name: { $regex: orgFilter, $options: 'i' } } : {};
  ...
  for (const org of orgs) {
    ...
    if (integrations.slack?.accessToken) {
      ...
      if (integrations.slack.immediateInsights) {
        ...
        console.log(
          `       Users: ${integrations.slack.immediateInsights?.stats?.activeUsers || 'N/A'}`
        );
      }
    } else {
    ...
    if (integrations.google?.accessToken) {
      ...
      if (integrations.google.immediateInsights) {
        console.log('       Immediate insights: Available');
        console.log(
          `       Meetings this week: ${integrations.google.immediateInsights?.stats?.meetingsThisWeek || 'N/A'}`
        );
      }
    } else {
