...
console.log(
  `   This will: sync events → compute metrics → generate signals → bridge to dashboard\n`
);
...
for (const r of results) {
  if (r.success) {
    console.log(
      `  ✅ ${r.source}: ${r.eventsProcessed || 0} events (${r.eventsCreated || 0} new, ${r.eventsUpdated || 0} updated)`
    );
  } else {
