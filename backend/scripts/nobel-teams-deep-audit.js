  ...
  for (const ch of channels) {
    try {
      ...
      if (msgs.length > 0) {
        ...
        console.log(`   📝 #${ch.displayName}: ${msgs.length} messages (${recentMsgs.length} recent)`);
        ...
        for (const m of recentMsgs) {
          ...
          console.log(`      → ${m.createdDateTime} | ${m.from?.user?.displayName || 'unknown'}: ${preview}`);
        }
      } else {
...
console.log(`Messages in DB (last 14 days): ${allTeamsEvents.filter(e => e.timestamp >= twoWeeksAgo).length}`);
