...
export function calculateConfidence(factors) {
  ...
  const confidence =
    0.25 * dataCoverage + 0.25 * baselineConfidence + 0.25 * sustainFactor + 0.25 * sourceQuality;
...
export const SIGNAL_TEMPLATES = {
  ...
  recovery_gap_index: {
    ...
    consequences: {
      ...
      CRITICAL:
        'Recovery time has collapsed. High risk of fatigue-driven errors and disengagement.',
    },
    ...
    // Top driver keys
    driverKeys: ['after_hours_activity_rate', 'late_meetings_count', 'early_meetings_count'],
  ...
  focus_fragmentation: {
    ...
    thresholds: {
      ...
      RISK: {
        rules: [
          {
            metric: 'days_with_focus_block_pct',
            condition: 'drop >= 25pp OR robust_z >= +1.5',
            sustained: 2,
          },
      ...
      CRITICAL: {
        rules: [
          {
            metric: 'days_with_focus_block_pct',
            condition: 'drop >= 35pp OR robust_z >= +2.0',
            sustained: 3,
          },
    ...
    driverKeys: ['back_to_back_rate', 'meeting_hours_per_day', 'recurring_meeting_hours_share'],
  ...
  meeting_load_drift: {
    ...
    thresholds: {
      ...
      RISK: {
        rules: [
          {
            metric: 'delta_pct_vs_baseline',
            condition: 'value >= 15 OR drift_slope >= 0.35',
            sustained: 2,
          },
        ],
      },
      CRITICAL: {
        rules: [
          {
            metric: 'delta_pct_vs_baseline',
            condition: 'value >= 25 OR drift_slope >= 0.5',
            sustained: 3,
          },
        ],
      },
  ...
  responsiveness_pressure: {
    ...
    driverKeys: ['urgent_channel_share', 'after_hours_message_share', 'messages_per_active_user'],
  ...
  engagement_asymmetry: {
    ...
    thresholds: {
      ...
      RISK: {
        rules: [
          {
            metric: 'top20_share',
            condition: 'increase >= 15pp OR robust_z >= +1.5',
            sustained: 2,
          },
        ],
      },
      CRITICAL: {
        rules: [
          {
            metric: 'top20_share',
            condition: 'increase >= 20pp OR robust_z >= +2.0',
            sustained: 3,
          },
        ],
      },
    ...
    consequences: {
      ...
      CRITICAL:
        'Collaboration is carried by a shrinking subset. High risk of burnout for carriers and withdrawal for others.',
    },
  ...
  signal_convergence: {
    ...
    description:
      'Escalate when multiple independent risk signals move together (system-level issue)',
    ...
    thresholds: {
      INFO: {
        rules: [{ metric: 'active_count', condition: 'value >= 2', sustained: 1 }],
      },
      RISK: {
        rules: [
    ...
    consequences: {
      ...
      CRITICAL:
        'System-level overload. High likelihood of performance and wellbeing degradation unless structure changes.',
    },
