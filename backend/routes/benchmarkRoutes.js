...
router.get('/internal/:teamId', async (req, res) => {
  try {
    ...
    if (!team || !team.bdiHistory)
      return res.status(404).json({ message: 'Team or history not found' });
    ...
    const bdi3m = avg(
      team.bdiHistory.filter((h) => new Date(h.date) >= monthsAgo(3)).map((h) => h.bdi)
    );
    const bdi12m = avg(
      team.bdiHistory.filter((h) => new Date(h.date) >= monthsAgo(12)).map((h) => h.bdi)
    );
