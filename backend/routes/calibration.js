 ...
 */
router.get('/status/:orgId', authenticateToken, requireOrganizationAccess(), async (req, res) => {
  try {
 ...
 */
router.post('/start/:orgId', authenticateToken, requireOrganizationAccess(), async (req, res) => {
  try {
    const { orgId } = req.params;
    const org = await Organization.findById(orgId);
 ...
 */
router.get('/progress/:orgId', authenticateToken, requireOrganizationAccess(), async (req, res) => {
  try {
    const { orgId } = req.params;
    const org = await Organization.findById(orgId);
    ...
    const daysSinceStart = org.calibration.calibrationStartDate
      ? Math.floor(
          (Date.now() - new Date(org.calibration.calibrationStartDate).getTime()) /
            (1000 * 60 * 60 * 24)
        )
      : 0;
