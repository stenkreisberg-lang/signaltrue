  ...
  try {
    ...
    if (!payload.signals || payload.signals.length === 0) {
      return res.json({
        ...
        confidence_notes: [
          'No signals to analyze. This may indicate healthy patterns or insufficient data.',
        ],
        safety: {
  ...
  try {
    ...
    if (!connectors?.jira?.connected && !connectors?.asana?.connected) {
      recommendations.push({
        ...
        recommendation:
          'Connect Jira or Asana to enable execution metrics (completion rate, WIP, cycle time)',
        impact: 'high',
      ...
      sections: [
        {
          ...
          content:
            'SignalTrue analyzes metadata only—timestamps, counts, and patterns. We never read email content, document text, or chat messages.',
        },
        {
          heading: 'How We Detect Signals',
          content:
            "We compare current metrics against your organization's own baseline (trailing 28 days). When patterns deviate significantly, we flag them as signals.",
        },
        {
          heading: 'How We Calculate Severity',
          content:
            'Severity (0-100) is based on how far metrics have deviated from baseline using robust statistical methods that resist outliers.',
        },
        {
          heading: 'How We Calculate Confidence',
          content:
            'Confidence reflects data quality: which integrations are connected, user mapping coverage, and whether multiple sources agree.',
        },
        {
          heading: 'How Actions Are Selected',
          content:
            'Actions come from a research-backed playbook, not AI generation. The AI only helps explain patterns in plain language.',
        },
        {
          heading: 'Research Backing',
          content:
            'Our signals are grounded in the Job Demands-Resources (JD-R) model, after-hours email research, and meeting fatigue studies.',
        },
      ],
      privacy_badge: 'Metadata only. No message content read.',
      compliance_note:
        'SignalTrue is designed for ISO/IEC 42001 alignment (AI management standard).',
    });
  } catch (err) {
...
function buildExplanation(signal, playbook) {
  const researchBacking = {
    recovery_collapse:
      'Based on meeting fatigue research showing that transition time between meetings is crucial for cognitive recovery.',
    execution_stagnation:
      'Based on the JD-R model which links sustained high demands with accumulating work to exhaustion.',
    rework_spiral: 'Based on quality research showing rework creates demand without adding value.',
    boundary_erosion:
      'Based on research linking after-hours email expectations to emotional exhaustion.',
    meeting_fatigue: 'Based on virtual meeting fatigue research highlighting recovery needs.',
    decision_churn:
      'Based on decision fatigue research showing repeated deliberation drains cognitive resources.',
  };

  const confidenceNote =
    signal.confidence < 60 ? 'This is an early pattern, not a conclusion. ' : '';
