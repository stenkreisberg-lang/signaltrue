...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const recentBdis = team.bdiHistory
        .slice(0, 8)
        .map((h) => h.bdi)
        .reverse();
      ...
      for (let week = 1; week <= parseInt(weeksAhead); week++) {
        const predictedBdi = Math.round(
          model.intercept + model.slope * (recentBdis.length + week - 1)
        );
        forecasts.push({
          week,
  ...
  async (req, res) => {
    try {
      ...
      for (const team of teams) {
        ...
        const week4Bdi = Math.max(
          0,
          Math.min(100, Math.round(model.intercept + model.slope * (recentBdis.length + 3)))
        );
