...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="signaltrue-metrics-${Date.now()}.csv"`
      );
      res.send(csv.join('\n'));
    } catch (e) {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const start = startDate ? new Date(startDate) : new Date(Date.now() - 30 * 24 * 3600 * 1000);
      const end = endDate ? new Date(endDate) : new Date();
      const events = await DriftEvent.find({ orgId, date: { $gte: start, $lte: end } }).sort({
        date: -1,
      });
      const csv = [
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="signaltrue-drift-${Date.now()}.csv"`
      );
      res.send(csv.join('\n'));
    } catch (e) {
      res.status(500).json({ message: e.message });
