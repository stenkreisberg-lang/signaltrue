...
router.post('/submit', async (req, res) => {
  try {
    ...
    console.log(
      `[Drift Diagnostic] New session: ${session.sessionId} | Score: ${score.totalScore} | Category: ${score.category}`
    );
...
router.post('/unlock', async (req, res) => {
  try {
    ...
    console.log(
      `[Drift Diagnostic] Session unlocked: ${sessionId} | Email: ${email} | Personal: ${isPersonalEmail}`
    );
...
router.get('/stats', async (req, res) => {
  try {
    ...
    const conversionRate =
      totalSessions > 0 ? ((unlockedSessions / totalSessions) * 100).toFixed(1) : 0;
