...
function generateNudges(teamMetrics, baseline, teamName) {
  ...
  if (afterHoursRate > baselineAfterHours * 1.25) {
    const percentIncrease = Math.round(
      ((afterHoursRate - baselineAfterHours) / baselineAfterHours) * 100
    );
    nudges.push({
...
function generateScorecard(teamMetrics, baseline) {
  ...
  const overallScore = Math.round(
    0.3 * teamHealthScore + 0.25 * responseScore + 0.25 * oneOnOneScore + 0.2 * sentimentScore
  );
...
function generateOneOnOneAgenda(teamMetrics, baseline) {
  ...
  if (focusTime < baselineFocus * 0.9) {
    agenda.push({
      ...
      suggestedQuestion:
        'How has your ability to do deep, focused work been lately? Are there interruptions or context switches we could reduce?',
      priority: 'medium',
    });
  }
  ...
  agenda.push({
    ...
    suggestedQuestion:
      "What's something you've accomplished recently that you're proud of? And what's something you'd like to learn or work on next?",
    priority: 'low',
  });
...
router.get('/:teamId', authenticateToken, async (req, res) => {
  try {
    ...
    const afterHoursChange = Math.round(
      ((avgMetrics.afterHoursRate - baseline.afterHoursRate) / baseline.afterHoursRate) * 100
    );
    ...
    const meetingChange = Math.round(
      ((avgMetrics.meetingLoadIndex - baseline.meetingLoadIndex) / baseline.meetingLoadIndex) * 100
    );
    ...
    const focusChange = Math.round(
      ((avgMetrics.focusTimeRatio - baseline.focusTimeRatio) / baseline.focusTimeRatio) * 100
    );
