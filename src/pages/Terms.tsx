        ...
        <section className="py-16">
          <div className="container mx-auto max-w-4xl space-y-5 px-6">
            {terms.map(([title, copy]) => (
              <article
                key={title}
                className="rounded-container border border-[#E2E8F0] bg-white p-7"
              >
                <h2 className="text-lead font-bold text-[#0F172A]">{title}</h2>
                <p className="mt-3 leading-7 text-[#475569]">{copy}</p>
              </article>
            ))}
            <p className="text-caption leading-6 text-[#64748B]">
