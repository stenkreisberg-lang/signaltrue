...
export default function CeoSummaryPage() {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center px-6">
        <div className="max-w-md text-center">
          ...
          <h1 className="mt-5 text-lead font-bold text-slate-900">Executive brief unavailable</h1>
  ...
  return (
    <div className="min-h-screen bg-slate-100 py-8 print:bg-white print:py-0">
      ...
      <main className="mx-auto max-w-4xl rounded-container border border-slate-200 bg-white p-7 shadow-sm print:border-0 print:shadow-none sm:p-10">
        <header className="border-b border-slate-200 pb-8">
          <div className="flex flex-wrap items-start justify-between gap-5">
            <div>
              <div className="flex items-center gap-3 text-caption text-slate-600">
                <Building2 className="h-5 w-5" /> Prepared for {summary.organizationName}
              </div>
              <h1 className="mt-4 text-section font-bold tracking-tight text-slate-950 sm:text-display">
            ...
            <button
              ...
              className="no-print inline-flex items-center gap-2 rounded-control bg-slate-900 px-4 py-2.5 text-caption font-bold text-white"
            >
              <Printer className="h-4 w-4" /> Print or save PDF
            </button>
          </div>
          <div className="mt-5 flex flex-wrap gap-4 text-caption text-slate-500">
        ...
        <section className="py-8">
          <p className="text-caption font-bold uppercase tracking-wider text-blue-700">
            Decision status
          </p>
          <div className="mt-3 rounded-container bg-slate-950 p-6 text-white">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <p className="text-caption text-slate-300">Exposure direction</p>
                <p className="mt-1 text-lead font-bold capitalize">
                  {summary.riskDirection.overall}
                </p>
              </div>
              <div className="rounded-full border border-slate-600 px-3 py-1 text-caption font-bold capitalize">
        ...
        <section className="border-t border-slate-200 py-8">
          <h2 className="text-lead font-bold text-slate-950">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-3">
            {observations.map(([title, observation]) => (
              <article
                key={title}
                className="rounded-container border border-slate-200 bg-slate-50 p-5"
              >
                <div className="flex items-center gap-2">
                  <DirectionIcon direction={observation.direction} />
                  <h3 className="font-bold text-slate-900">{title}</h3>
                </div>
                <p className="mt-3 text-caption leading-6 text-slate-600">{observation.summary}</p>
                {observation.percentChange != null && (
                  <p className="mt-3 text-caption font-bold uppercase tracking-wide text-slate-500">
        ...
        <section className="border-t border-slate-200 py-8">
          <h2 className="text-lead font-bold text-slate-950">Why leadership should review this</h2>
          <p className="mt-4 rounded-container border border-blue-200 bg-blue-50 p-5 leading-7 text-slate-700">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-3">
            {[
              ...
              <div key={title} className="rounded-container border border-slate-200 p-4">
                <h3 className="font-bold text-slate-900">{title}</h3>
                <p className="mt-2 text-caption leading-6 text-slate-600">{copy}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="border-t border-slate-200 py-8">
          <div className="flex items-start gap-3 rounded-container border border-emerald-200 bg-emerald-50 p-5">
            <ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-emerald-700" />
            <div>
              <h2 className="font-bold text-emerald-950">Use boundaries</h2>
              <div className="mt-3 grid gap-2 text-caption leading-6 text-slate-700">
                {[
        ...
        <footer className="border-t border-slate-200 pt-6 text-caption text-slate-500">
          {summary.footer}
        </footer>
      </main>
    </div>
