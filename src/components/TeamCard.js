  ...
  return (
    <div style={modalOuterStyle}>
      <div style={modalInnerStyle}>
        <h2 style={{ fontSize: isMobile ? 18 : 20, fontWeight: 700, marginBottom: 12 }}>
        ...
        <div style={{ marginBottom: 18 }}>
          ...
          <ul style={{ margin: '8px 0 0 0', padding: 0, listStyle: 'none' }}>
            {Object.keys(drivers).length === 0 && (
              <li style={{ color: '#9ca3af' }}>No driver weights set</li>
            )}
            {Object.entries(drivers).map(([k, v]) => (
              <li key={k} style={{ marginBottom: 6 }}>
                ...
                <input
                  ...
                  min="0"
                  style={inputStyle}
                  onChange={(e) => handleDriverChange(k, parseFloat(e.target.value))}
                />
              </li>
            ))}
          </ul>
        ...
        <div style={{ marginBottom: 18 }}>
          ...
          <ul style={{ margin: '8px 0 0 0', padding: 0, listStyle: 'none' }}>
            {Object.keys(seasonality).length === 0 && (
              <li style={{ color: '#9ca3af' }}>No seasonality flags</li>
            )}
            {Object.entries(seasonality).map(([k, v]) => (
              <li key={k} style={{ marginBottom: 6 }}>
                {k}:{' '}
                <button
                  onClick={() => handleSeasonalityToggle(k)}
                  style={{
                    ...buttonStyle,
                    background: v ? '#10b981' : '#d1d5db',
                    color: v ? 'white' : dark ? '#d1d5db' : '#374151',
                  }}
                >
                  {v ? 'On' : 'Off'}
                </button>
          ...
          <ul style={{ margin: '8px 0 0 0', padding: 0, listStyle: 'none' }}>
            ...
            {deltas.map((d) => (
              <li key={d.key}>
                ...
                <span
                  style={{
                    color: d.delta > 0 ? '#10b981' : d.delta < 0 ? '#ef4444' : '#6b7280',
                    fontWeight: 600,
                  }}
                >
                  ({d.delta > 0 ? '+' : ''}
        ...
        <div style={flexButtonsStyle}>
          <button onClick={onClose} disabled={saving} style={closeButtonStyle}>
            Close
          </button>
        ...
        >
          <div>
            ...
            {baseline && (
              ...
              >
                {baseline.percentChange <= 0 ? '↓' : '↑'}{' '}
                {Math.abs(baseline.percentChange).toFixed(1)}% vs baseline •{' '}
                {baseline.daysSinceBaseline}d
              </p>
            )}
          </div>
        ...
        >
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            ...
            <button
              ...
              onMouseEnter={(e) =>
                (e.currentTarget.style.background = 'linear-gradient(135deg, #059669, #2563eb)')
              }
              onMouseLeave={(e) =>
                (e.currentTarget.style.background = 'linear-gradient(135deg, #10b981, #60a5fa)')
              }
            >
              🧮 Drivers
            </button>
