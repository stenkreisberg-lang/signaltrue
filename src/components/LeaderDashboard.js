...
export default function LeaderDashboard({ teamId, dark }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE}/api/leader/dashboard/${teamId}`, {
  ...
  if (loading)
    return <div style={{ color: dark ? '#9ca3af' : '#6b7280' }}>Loading leader dashboard...</div>;
  if (!data) return null;
  ...
  const statusColor =
    data.metrics.status === 'improving'
      ? '#10b981'
      : data.metrics.status === 'declining'
        ? '#ef4444'
        : '#f59e0b';
    ...
    <div style={containerStyle}>
      ...
      <div style={metricsStyle}>
        <div style={metricCardStyle}>
          <div
            style={{ fontSize: '12px', color: dark ? '#9ca3af' : '#6b7280', marginBottom: '4px' }}
          >
            BDI
          </div>
          <div style={{ fontSize: '28px', fontWeight: '700' }}>{data.metrics.bdi}</div>
          <div style={{ fontSize: '11px', color: dark ? '#d1d5db' : '#4b5563' }}>
            {data.metrics.zone}
          </div>
        </div>
        <div style={metricCardStyle}>
          ...
          <div style={{ fontSize: '28px', fontWeight: '700', color: statusColor }}>
      ...
      <div>
        <h3 style={{ fontSize: '18px', fontWeight: '600', marginBottom: '12px' }}>
          📋 Your Top 3 Leadership Focuses This Week
        </h3>
        <div
          style={{
