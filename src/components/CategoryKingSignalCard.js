...
export default function CategoryKingSignalCard({
  ...
  return (
    <div
      className={`rounded-lg border ${severityConfig.border} ${severityConfig.bg} overflow-hidden ${className}`}
    >
      {/* Header */}
      <div
      ...
      >
        <div className="flex items-center gap-3">
          <div
            className={`w-10 h-10 rounded-lg bg-${metadata.color}-100 flex items-center justify-center`}
          >
...
function getResearchBacking(signalType) {
  const research = {
    recovery_collapse:
      'Based on meeting fatigue research showing cognitive recovery needs 5-15 minutes between focused activities.',
    execution_stagnation:
      'Based on the JD-R model linking sustained high demands with accumulating work to exhaustion.',
    rework_spiral: 'Based on quality research showing rework creates demand without adding value.',
    boundary_erosion:
      'Based on research linking after-hours email expectations to emotional exhaustion (Barber & Santuzzi, 2015).',
    meeting_fatigue:
      'Based on virtual meeting fatigue research highlighting recovery needs (Bailenson, 2021).',
    decision_churn:
      'Based on decision fatigue research showing repeated deliberation drains cognitive resources.',
    wip_overload:
      'Based on flow research showing context-switching costs can consume 20-40% of productive time.',
    panic_coordination:
      'Based on crisis management research linking reactive meetings to underlying process failures.',
    external_pressure_injection:
      'Based on boundary theory research on external demands disrupting internal workflows.',
    overcommitment_risk:
      'Based on planning fallacy research showing systematic underestimation of effort required.',
  };
