(function () {
  ...
  const GA_MEASUREMENT_ID = 'G-32VLC15W5G';
  ...
  function analyticsEligible() {
    ...
    const debug = ['1', 'true', 'yes'].includes((params.get('debug_mode') || '').toLowerCase());
    return (
      location.hostname.toLowerCase() === COMMERCIAL_HOSTNAME &&
      !navigator.webdriver &&
      !automated &&
      !debug &&
      !/(?:bot|crawler|spider|headlesschrome|lighthouse|playwright|puppeteer)/i.test(
        navigator.userAgent
      )
    );
  }
  ...
  function initializeAnalytics() {
    ...
    if (!document.getElementById('signaltrue-ga4')) {
      const script = document.createElement('script');
      script.id = 'signaltrue-ga4';
      script.async = true;
  ...
  function safePath(value) {
    try {
      ...
      [
        ...
        if (url.searchParams.get(key)) search.set(key, url.searchParams.get(key));
      });
      return `${url.pathname}${search.toString() ? `?${search}` : ''}`;
    } catch (_) {
      return '/drift/run.html';
    }
  }
  ...
  function getUtm() {
    ...
    utm.referrer = safeReferrer(document.referrer || '') || null;
    return utm;
  }
  ...
  const steps = [
    ...
    },
    {
      ...
      title: 'Recovery gaps',
      subtitle:
        'How often do people get real buffers between heavy work blocks (meetings, deadlines, intense collaboration)?',
      type: 'choices',
      options: [
  ...
  function score(answers) {
    ...
    // Subscores
    const subScores = {
      meeting_pressure: Math.round(
        ((map.meeting_time[answers.meeting_time] + map.back_to_back[answers.back_to_back]) / 24) *
          100
      ),
      response_pressure: Math.round(
        (map.response_expectations[answers.response_expectations] / 12) * 100
      ),
    ...
    const findings = [];
    if (subScores.meeting_pressure >= 70)
      findings.push('Meeting load is compressing recovery and focus time.');
    if (subScores.response_pressure >= 70)
      findings.push('Fast-response norms create constant interruption pressure.');
    if (subScores.focus_fragmentation >= 70)
      findings.push('Context switching is likely slowing execution even if effort stays high.');
    if (subScores.recovery_deficit >= 70)
      findings.push('Lack of buffers makes burnout risk structural, not personal.');
    if (subScores.urgency_culture >= 70)
      findings.push('Escalation-driven work rhythms create coordination drag.');

    if (findings.length === 0)
      findings.push(
        'No major drift hotspots detected. Keep watching for baseline shifts during growth or change.'
      );
  ...
  function render() {
    ...
    if (!isDone) {
      ...
      app.innerHTML = `
        <div>
          ...
          <p class="sub">7–10 minutes. No employee data or surveillance. This flags system-level risk patterns, not individual performance.</p>
          ...
          <div class="notice">
            The diagnostic questions collect no names, content, or employee-level data. A work email is requested only after the preview if you choose to unlock the full report.
          </div>
      ...
      el('#nextBtn').addEventListener('click', async () => {
        ...
        if (state.step < steps.length - 1) {
          state.step++;
          render();
          track('diagnostic_step_view', { step: state.step + 1 });
          return;
        }
        // finish
        ...
        <div style="margin-top:16px">
          ...
          <div class="field">
            <label style="display:flex; gap:10px; align-items:flex-start; color: var(--muted); font-size:12px; line-height:1.4">
              ...
              <span>Send me occasional follow-up guidance. Optional; unsubscribe anytime.</span>
            </label>
          </div>
    ...
    el('#unlockBtn').addEventListener('click', async () => {
      ...
      try {
        track('diagnostic_unlock_submit', { diagnostic_session_id: state.sessionId });
        ...
        await track('diagnostic_lead_confirmed', {
  ...
  function track(name, props) {
    try {
      const context = {
        page_path: safePath(`${location.pathname}${location.search}`),
      };
      const data = { ...context, ...(props || {}) };
      if (initializeAnalytics() && typeof window.gtag === 'function') {
        window.gtag('event', name, { ...data, transport_type: 'beacon' });
